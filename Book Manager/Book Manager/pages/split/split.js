(function () {
    ...
    var openBook = function (bookId) {
        var options = new Windows.System.LauncherOptions();
        options.displayApplicationPicker = true;

        storagePermissions.futureAccessList.getFileAsync(bookId).then(function (file) {
    ...
    ui.Pages.define("/pages/split/split.html", {
        ...
        ready: function (element, options) {
            var buttonOpenBook = element.querySelector(".open-book");
            buttonOpenBook.addEventListener("click", function () {
                var bookId = element.querySelector(".book-id").firstChild.data;
                openBook(bookId)
            });


            var showResumeButton = document.getElementById("show-resume");
            showResumeButton.addEventListener("click", function () {
                document.getElementById("book-resume").className = "visible";
                document.getElementById("book-notes").className = "hidden";
            });

            var showNotesButton = document.getElementById("show-notes");
            showNotesButton.addEventListener("click", function () {
                document.getElementById("book-notes").className = "visible";
                document.getElementById("book-resume").className = "hidden";
            });
            ...
            var ratingControl = element.querySelector("#ratingControl").winControl;
            //ratingControl.addEventListener("change", this.ratingChanged);
            ...
            if (this._isSingleColumn()) {
                if (this._itemSelectionIndex >= 0) {
                    ...
                    binding.processAll(element.querySelector(".booksection"), this._items.getAt(this._itemSelectionIndex));
                }
            } else {
        ...
        ratingChanged: function (eventInfo) {
            var ratingOutputDiv = document.getElementById("ratingOutput");
        ...
        updateLayout: function (element, viewState, lastViewState) {
            ...
            if (this._isSingleColumn()) {
                ...
                if (this._itemSelectionIndex >= 0) {
                    ...
                    element.querySelector(".booksection").focus();
                } else {
            ...
            listView.selection.getItems().done(function updateDetails(items) {
                if (items.length > 0) {
                    ...
                    if (this._isSingleColumn()) {
                        ...
                        details = document.querySelector(".booksection");
        ...
        _updateVisibility: function () {
            ...
            if (this._isSingleColumn()) {
                if (this._itemSelectionIndex >= 0) {
                    utils.addClass(document.querySelector(".booksection"), "primarycolumn");
                    document.querySelector(".booksection").focus();
                } else {
